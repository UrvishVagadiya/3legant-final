...
export default function SignUpPage() {
  ...
  const onSubmit: SubmitHandler<FormInputs> = async (data) => {
    ...
    if (error) {
      ...
      alert(`Signup failed: ${error.message}${error.status ? ` (Status: ${error.status})` : ""}`);
    } else {
  ...
  return (
    <AuthLayout>
      ...
      <form
        ...
        className="space-y-2 md:space-y-6 flex flex-col font-poppins w-full"
      >
        <div>
          <input
        ...
        <div>
          ...
          />
          {errors.username && (
            <p className="pt-2 text-red-500 text-xs">{errors.username.message}</p>
          )}
        </div>
        ...
        <div>
          ...
          />
          {errors.password && (
            <p className="pt-2 text-red-500 text-xs">{errors.password.message}</p>
          )}
        </div>
