...
interface Payment {
  id: string; order_id: string; payment_method: string; status: string;
  transaction_id: string | null; amount: number; currency: string;
...
export default function AdminPayments() {
  ...
  const filtered = payments.filter((p) => {
    ...
    const matchSearch = (p.order_code || "").toLowerCase().includes(q) || (p.customer_name || "").toLowerCase().includes(q) || (p.transaction_id || "").toLowerCase().includes(q);
    return matchSearch && (statusFilter === "all" || p.status === statusFilter);
  });

  const totalRevenue = payments.filter((p) => p.status === "completed" || p.status === "refunded").reduce((s, p) => s + Number(p.amount), 0);
  const totalRefunded = payments.filter((p) => p.refund_amount).reduce((s, p) => s + Number(p.refund_amount || 0), 0);
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[{ label: "Total Payments", val: String(payments.length), color: "text-[#141718]" }, { label: "Total Revenue", val: `$${totalRevenue.toFixed(2)}`, color: "text-green-600" }, { label: "Total Refunded", val: `$${totalRefunded.toFixed(2)}`, color: "text-red-600" }].map((s) => (
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        <div className="relative flex-1 max-w-md">
          ...
          <input type="text" placeholder="Search by order, customer, or transaction ID..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-lg text-sm outline-none focus:border-[#141718]" />
        </div>
        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="border border-gray-200 rounded-lg px-4 py-2.5 text-sm outline-none focus:border-[#141718]">
          <option value="all">All Statuses</option><option value="pending">Pending</option><option value="completed">Completed</option><option value="failed">Failed</option><option value="refunded">Refunded</option>
        </select>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-gray-50 text-[#6C7275]">
              <tr>{["Order", "Customer", "Method", "Amount", "Status", "Transaction ID", "Date", "Actions"].map((h) => <th key={h} className="text-left px-6 py-3 font-medium">{h}</th>)}</tr>
            </thead>
            <tbody>
              {filtered.map((p) => (
                <tr key={p.id} className="border-t border-gray-100 hover:bg-gray-50">
                  <td className="px-6 py-4 font-medium text-[#141718]">{p.order_code}</td>
                  <td className="px-6 py-4 text-[#141718]">{p.customer_name}</td>
                  <td className="px-6 py-4 text-[#6C7275] capitalize">
                    {p.payment_method?.replace(/_/g, " ")}
                    {p.card_last_four && <span className="block text-xs text-gray-400">{p.card_brand && `${p.card_brand} `}•••• {p.card_last_four}</span>}
                  </td>
              ...
              {filtered.length === 0 && <tr><td colSpan={8} className="px-6 py-12 text-center text-[#6C7275]">No payments found</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
