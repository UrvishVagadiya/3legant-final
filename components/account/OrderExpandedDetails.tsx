          ...
          <div className="border-t border-gray-100 pt-4 flex flex-wrap items-center justify-between gap-4">
            {order.tracking_number && (
              <div className="flex flex-col">
                <span className="text-[10px] text-[#6C7275] uppercase font-bold tracking-wider mb-0.5">Tracking Number</span>
                <span className="font-mono text-sm text-[#141718]">{order.tracking_number}</span>
              </div>
            )}
            ...
            <div className="flex items-center gap-3">
              ...
              {order.refund_status !== "none" && (
                <div className={`flex items-center gap-2 px-4 py-2 border rounded-full text-sm font-medium shadow-sm capitalize ${
                  order.refund_status === 'requested' ? 'border-yellow-200 bg-yellow-50 text-yellow-700' :
                  order.refund_status === 'approved' ? 'border-green-200 bg-green-50 text-green-700' :
