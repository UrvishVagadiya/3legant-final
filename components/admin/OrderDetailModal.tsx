...
export default function OrderDetailModal({
  ...
  const handleRefundAction = async (action: 'approve' | 'reject') => {
    ...
    try {
      ...
      onUpdateStatus(order.id, action === 'approve' ? 'refunded' : order.status);
    } catch (error: any) {
            ...
            <div className="p-6 bg-gray-50 rounded-xl border border-gray-200 space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-semibold text-[#141718] flex items-center gap-2">
                  <RefreshCcw size={20} className={order.refund_status === 'requested' ? 'animate-pulse text-yellow-600' : ''} />
                  Refund Request
                </h3>
              ...
              {order.refund_status === 'requested' && (
                <div className="flex gap-3 pt-2">
                  ...
                  >
                    {updatingRefund ? <Loader2 size={16} className="animate-spin" /> : 'Approve & Process Refund'}
                  </button>
                  <button
                    disabled={updatingRefund}
