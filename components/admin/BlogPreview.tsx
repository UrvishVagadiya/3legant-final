...
const MDXComponents = {
  ...
  h2: ({ children }: any) => <h2 className="text-[26px] md:text-[32px] font-medium mt-10 mb-4 text-[#141718]">{children}</h2>,
  h3: ({ children }: any) => <h3 className="text-xl md:text-2xl font-medium mt-8 mb-3 text-[#141718]">{children}</h3>,
  ...
  Row: ({ children }: any) => <div className="grid grid-cols-1 md:grid-cols-2 gap-4 my-8">{children}</div>,
};
  ...
  return (
    <div className="bg-white min-h-screen">
      <div className="sticky top-0 z-10 bg-white border-b border-gray-100 px-6 py-3 flex items-center justify-between">
        <button onClick={onBack} className="flex items-center gap-2 text-sm font-medium text-[#141718] hover:opacity-70">
          <FiArrowLeft /> Back to Editor
        </button>
        <div className="text-xs font-bold text-[#38CB89] uppercase tracking-widest">Live Preview Mode</div>
      </div>

      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 font-poppins text-[#141718]">
        ...
        <div className="space-y-4 mb-8 md:mb-10 w-full lg:w-[90%]">
          <span className="text-xs font-bold uppercase tracking-wider text-[#6C7275]">ARTICLE</span>
