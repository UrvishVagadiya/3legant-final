...
const blogs = [
  ...
  <br/>
  <h3>Vintage Finds</h3>
  <div>The loft is furnished with a mix of modern pieces and industrial antiques, giving the space a lived-in and authentic feel.</div>
</div>`
  },
  {
    id: 10,
...
async function setup() {
  console.log("Starting unique blog seeding...");
  ...
  for (const blog of blogs) {
    ...
    if (error) {
      ...
      console.log(`Successfully seeded blog ${blog.id}: ${blog.title}`);
    }
  }
