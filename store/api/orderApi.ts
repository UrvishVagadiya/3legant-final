    ...
    getOrders: builder.query<Order[], string>({
      queryFn: async (userId) => {
        ...
        const { data, error } = await supabase
          .from("orders")
          .select(
            "id, order_code, created_at, status, subtotal, shipping_cost, discount, total, shipping_method, tracking_number, payments(status), refund_status, refund_request_reason, refund_requested_at, delivered_at, order_items(id, product_name, product_image, color, quantity, unit_price, total_price)",
          )
          .eq("user_id", userId)
